...
interface Props {
  ...
  onOpenInvite: () => void;
  onCreateItinerary: (tripData: { destination: string; dates: string; activities: ActivityItem[]; travelers: number; title: string; daysCount?: number; tripId: string; members: TripMember[] }) => void;
...
export const TripCustomizerView: React.FC<Props> = ({ onNavigate, onOpenInvite, onCreateItinerary, initialDestination }) => {
  ...
  const createChat = async () => {
    if (!name.trim()) { setError('Enter a trip name first.'); return; }
    setCreating(true); setError('');
    try {
      const trip = await createTripChat(name.trim(), members.map((m) => m.id));
      setTripId(trip.id); setMembers(trip.members || members); setSetup(false);
  ...
  if (!setup) return <TripCustomizerLegacy
    ...
    onCreateItinerary={(data) => onCreateItinerary({ ...data, title: name, tripId, members })}
  />;

  return <div className="max-w-3xl mx-auto pb-20">
    <div className="text-center pt-4 mb-8"><span className="text-xs font-semibold uppercase tracking-[0.16em] text-orange-700">New trip</span><h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight mt-2">Create your trip squad</h1><p className="text-sm text-slate-600 max-w-xl mx-auto mt-2">Name the trip, create its chat, add verified TripTailor users, then customise your itinerary.</p></div>
    <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-7 shadow-xs space-y-6">
      ...
      <div className="pt-3 border-t border-slate-100 flex justify-between"><button onClick={() => onNavigate('dashboard')} className="px-4 py-2.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 rounded-xl cursor-pointer">Cancel</button><button disabled={creating || !name.trim()} onClick={createChat} className="px-5 py-2.5 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-2 cursor-pointer">{creating ? <RotateCw className="w-4 h-4 animate-spin" /> : <MessageSquare className="w-4 h-4" />}{creating ? 'Creating chat...' : 'Create Chat & Continue'}</button></div>
    </div>
  </div>;
};
