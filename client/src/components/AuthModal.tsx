...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const res = await signupUser({ name: signupName, email: signupEmail, password: signupPassword, avatar: '', travelPace });
    setLoading(false);
    if (res.success && res.user) {
                ...
                <div className="grid grid-cols-3 gap-1.5 text-xs">
                  {(['Relaxed', 'Balanced', 'Packed'] as const).map((pace) => (
                    <button
                      ...
                      className={`py-1.5 rounded-xl font-bold transition-all border cursor-pointer ${travelPace === pace
