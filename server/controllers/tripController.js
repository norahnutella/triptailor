...
export async function listTrips(req, res) {
  try {
    ...
    res.status(500).json({ message: error.message || 'Unable to load trips.' });
  }
}
...
export async function createTrip(req, res) {
  try {
    ...
    res.status(400).json({ message: error.message || 'Unable to create trip.' });
  }
}
...
export async function getTrip(req, res) {
  try {
    ...
    res.status(500).json({ message: error.message || 'Unable to load trip.' });
  }
}
...
export async function updateTrip(req, res) {
  try {
    ...
    res.status(400).json({ message: error.message || 'Unable to update trip.' });
  }
}
...
export async function addMember(req, res) {
  try {
    ...
    res.status(400).json({ message: error.message || 'Unable to add member.' });
  }
}
...
export async function searchUsers(req, res) {
  try {
    ...
    res.status(500).json({ message: error.message || 'Unable to search users.' });
  }
}
...
export async function getMessages(req, res) {
  try {
    ...
    res.json({ messages: messages.map((message) => ({ id: message._id.toString(), text: message.text, createdAt: message.createdAt, sender: mapUser(message.sender) })) });
  } catch (error) {
    res.status(500).json({ message: error.message || 'Unable to load trip chat.' });
  }
}
...
export async function sendMessage(req, res) {
  try {
    ...
    const message = await TripMessage.create({ trip: trip._id, sender: req.user._id, text });
    await message.populate('sender', 'name email avatar role');
    res.status(201).json({ message: { id: message._id.toString(), text: message.text, createdAt: message.createdAt, sender: mapUser(message.sender) } });
  } catch (error) {
